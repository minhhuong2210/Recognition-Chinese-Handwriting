...
parser.add_argument('--test_dataset', type=str, default='C:/Users/hp/Documents/Nam3/NCKH/data_split/test_image', help='')
...
def test(epoch):
    ...
    for iteration in range(test_loader_len):
        ...
        batch = image.shape[0]
        pred = torch.zeros(batch,1).long()
        image_features = None
        prob = torch.zeros(batch, max_length).float()
        for i in range(max_length):
            length_tmp = torch.zeros(batch).long() + i + 1
        ...
        for i in range(batch):
            ...
            for j in range(max_length):
                if pred[i][j] != len(alphabet) - 1:
                    ...
                    break
            text_pred_list.append(torch.Tensor(now_pred)[1:].long())
